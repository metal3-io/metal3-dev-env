...
if [[ -n "${EXTERNAL_SUBNET_V4_HOST}" ]]; then
  export REGISTRY_IP="${REGISTRY_IP:-${EXTERNAL_SUBNET_V4_HOST}}"
  export REGISTRY="${REGISTRY:-${REGISTRY_IP}:${REGISTRY_PORT}}"
  export K8S_REGISTRY="${REGISTRY_IP}:5001"
else
  export REGISTRY_IP="${REGISTRY_IP:-${EXTERNAL_SUBNET_V6_HOST}}"
  export REGISTRY="${REGISTRY:-[${REGISTRY_IP}]:${REGISTRY_PORT}}"
  export K8S_REGISTRY="[${REGISTRY_IP}]:5001"
fi
# The domain is used only by the VMs in external network.
