...
export NUM_REDFISHVMNODES=${NUM_REDFISHVMNODES:-"0"}
...
if [ "$((NUM_NODES + NUM_REDFISHVMNODES))" -lt "$((NUM_OF_CONTROLPLANE_REPLICAS + NUM_OF_WORKER_REPLICAS))" ]; then
    echo "Failed with incorrect number of nodes"
    echo "NUM_NODES+NUM_REDFISHVMNODES: (${NUM_NODES} + ${NUM_REDFISHVMNODES}) < (${NUM_OF_CONTROLPLANE_REPLICAS} + ${NUM_OF_WORKER_REPLICAS})"
    exit 1
fi
