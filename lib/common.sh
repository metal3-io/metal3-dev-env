...
# By default CI deploys Ironic in-cluster via IRSO.
...
# When USE_IRSO=false, Ironic is deployed using the local dev tooling 
# IRONIC_RUN_LOCAL controls whether Ironic runs as local containers or
# in-cluster
...
if [[ "${CAPM3RELEASEBRANCH}" == "release-1.11" ]]; then
  ...
  export CAPM3BRANCH="${CAPM3BRANCH:-release-1.14}"
  export IPAMBRANCH="${IPAMBRANCH:-release-1.14}"
else
...
export REGISTRY_IMAGE_VERSION="3.1.1"
...
if [[ "${CAPM3RELEASEBRANCH}" = "release-1.11" ]]; then
  ...
  export BARE_METAL_OPERATOR_IMAGE=${BARE_METAL_OPERATOR_IMAGE:-"${CONTAINER_REGISTRY}/metal3-io/baremetal-operator:release-0.13"}
  export IRONIC_KEEPALIVED_IMAGE=${IRONIC_KEEPALIVED_IMAGE:-"${CONTAINER_REGISTRY}/metal3-io/keepalived:main"}
  ...
  export IPAM_IMAGE=${IPAM_IMAGE:-"${CONTAINER_REGISTRY}/metal3-io/ip-address-manager:release-1.14"}
  export BARE_METAL_OPERATOR_IMAGE=${BARE_METAL_OPERATOR_IMAGE:-"${CONTAINER_REGISTRY}/metal3-io/baremetal-operator:release-0.14"}
