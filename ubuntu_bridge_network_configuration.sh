#!/usr/bin/env bash

set -eux

# shellcheck disable=SC1091
source lib/logging.sh
# shellcheck disable=SC1091
source lib/common.sh
# shellcheck disable=SC1091
source lib/network.sh

if [[ "${MANAGE_PRO_BRIDGE}" = "y" ]]; then
    # Adding an IP address in the libvirt definition for this network results in
    # dnsmasq being run, we don't want that as we have our own dnsmasq, so set
    # the IP address here.
    # Create a veth iterface peer.
    ip link add ironicendpoint type veth peer name ironic-peer
    # Create provisioning bridge, if the user allowed bridged provisioning network.
    if [[ "${ENABLE_NATED_PROVISIONING_NETWORK:-false}" = "false" ]]; then
        brctl addbr provisioning
    fi
    # ifconfig provisioning 172.22.0.1 netmask 255.255.255.0 up
    # Use ip command. ifconfig commands are deprecated now.
    ip link set provisioning up
    if [[ "${BARE_METAL_PROVISIONER_SUBNET_IPV6_ONLY}" = "true" ]]; then
        ip -6 addr add "${BARE_METAL_PROVISIONER_IP}"/"${BARE_METAL_PROVISIONER_CIDR}" dev ironicendpoint
      else
        ip addr add dev ironicendpoint "${BARE_METAL_PROVISIONER_IP}"/"${BARE_METAL_PROVISIONER_CIDR}"
    fi
    brctl addif provisioning ironic-peer
    ip link set ironicendpoint up
    ip link set ironic-peer up

    # Need to pass the provision interface for bare metal
    if [[ -n "${PRO_IF}" ]]; then
        brctl addif provisioning "${PRO_IF}"
    fi
fi

if [[ "${MANAGE_INT_BRIDGE}" = "y" ]]; then
    # Create the external bridge
    if ! ip a show external &>/dev/null; then
        brctl addbr external
        # ifconfig external 192.168.111.1 netmask 255.255.255.0 up
        # Use ip command. ifconfig commands are deprecated now.
        if [[ -n "${EXTERNAL_SUBNET_V4_HOST}" ]]; then
            ip addr add dev external "${EXTERNAL_SUBNET_V4_HOST}/${EXTERNAL_SUBNET_V4_PREFIX}"
        fi
        if [[ -n "${EXTERNAL_SUBNET_V6_HOST}" ]]; then
            ip addr add dev external "${EXTERNAL_SUBNET_V6_HOST}/${EXTERNAL_SUBNET_V6_PREFIX}"
        fi
        ip link set external up
    fi

    # Add the internal interface to it if requests, this may also be the interface providing
    # external access so we need to make sure we maintain dhcp config if its available
    if [[ -n "${INT_IF}" ]]; then
        brctl addif "${INT_IF}"
    fi
fi

# restart the libvirt network so it applies an ip to the bridge
if [[ "${MANAGE_EXT_BRIDGE}" = "y" ]]; then
    virsh net-destroy external
    virsh net-start external
    # Need to bring UP the NIC after destroying the libvirt network
    if [[ -n "${INT_IF}" ]]; then
        ifup "${INT_IF}"
    fi
fi
