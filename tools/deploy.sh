...
# SCRIPTDIR points to the baremetal-operator repository root, which holds the
# kustomize bases/overlays and Makefile used below. Prefer BMOPATH (set by
# metal3-dev-env); otherwise fall back to the parent directory of this script so
# the script still works from within a baremetal-operator checkout.
if [ -n "${BMOPATH:-}" ]; then
    SCRIPTDIR="${BMOPATH}"
else
