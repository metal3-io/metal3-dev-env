...
| LOCAL_CONTAINER_PLATFORM | Locally built containers will be built for this os/arch platform | any string that is acceptable by the --platform flag of either docker or podman | "linux/amd64" |
...
| KUBERNETES_VERSION | Kubernetes version | "x.x.x" | "1.37.0" |
| UPGRADED_K8S_VERSION | Upgraded Kubernetes version | "x.x.x" | "1.37.0" |
...
| USE_IRSO | Deploy Ironic in-cluster using the ironic-standalone-operator (IRSO). This is the default. Set to "false" only for local development to deploy Ironic via the tools/deploy.sh or tools/run_local_ironic.sh scripts instead | "true","false" | "true" |
