...
# Clean up TLS certs and CSRs generated during setup. These may be owned by
# root (e.g. when created via sudo make), so rely on this script running as
# root to remove them.
rm -rf "${WORKING_DIR}"/certs
rm -rf /tmp/ironic.csr /tmp/ipxe.csr
rm -rf /opt/metal3/certs
