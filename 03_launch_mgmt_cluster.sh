...
deploy_ironic_and_or_bmo() {
  ...
  go build -o "${BMOPATH}/tools/deploy" "${BMOPATH}/tools/deploy.go"
  "${BMOPATH}/tools/deploy" "$@"
}
...
EOF
  ...
  # Deploy BMO using the deploy executable
...
EOF
  ...
  if [ "${EPHEMERAL_CLUSTER}" != "minikube" ]; then
    ...
    # Deploy Ironic using the deploy executable
